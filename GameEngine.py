...
class Game:
    ...
    def new(self):
        ...
        self.endFlag = EndFlag(self, 1500, screen_height/2)
        self.all_sprites.add(self.endFlag)
    ...
    def update(self):
        ...
        # Collisions with EndFlag
        end_collision = pg.sprite.collide_rect(self.player, self.endFlag)
    ...
    def draw(self):
        ...
        self.draw_text(str(int(self.player.position.x)), 12, WHITE, screen_width - 40, 60)
    ...
    def show_success_screen(self):
        ...
        if self.running == True:
            self.screen.fill(SPLASH)
            self.draw_text("Congratulations!", 48, WHITE, screen_width / 2, screen_height / 4)
            self.draw_text("Your High Score was " + str(self.score), 32, WHITE, screen_width / 2, screen_height / 4 + 70)
            pg.display.flip()
            self.timer = 600
            self.score = 0
            pg.time.delay(1000)
            self.draw_text("Press any key to return to try again", 22, WHITE, screen_width / 2, screen_height * 3 / 4)
            pg.display.flip()
            pg.event.clear()
            self.wait()
